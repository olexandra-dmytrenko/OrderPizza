...
public class AddressTest {
    ...
    public void addOneAddress() {
        ...
        Number numbOfLinesBeforeAdd = (Number) session.createCriteria("domain.Address")
                .setProjection(Projections.rowCount()).uniqueResult();
        int idBeforeSave = (int) session.createCriteria("domain.Address").setProjection(Projections.max("id"))
                .uniqueResult();
        ...
        Number numbOfLinesAfterAdd = (Number) session.createCriteria("domain.Address")
                .setProjection(Projections.rowCount()).uniqueResult();
    ...
    public void addSetOfAddresses() {
        ...
        try {
            // start transaction
            session.beginTransaction();
            numberOfRecordsBeforeAddition = ((Long) session.createQuery("SELECT COUNT(*) FROM domain.Address")
            ...
            numberOfRecordsAfterAddition = ((Long) session.createQuery("SELECT COUNT(*) FROM domain.Address")
                    .uniqueResult()).intValue();
            session.getTransaction().commit();
